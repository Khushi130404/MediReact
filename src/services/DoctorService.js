...
const DOCTOR_REGISTRATION_API = `${BASE_URL}/doctor/add_doctor`;
...
export const addDoctor = async (docInfo) => {
  try {
    console.log(docInfo);
    const response = await axios.post(DOCTOR_REGISTRATION_API, docInfo);
    console.log("Khushi");
    return response.data;
  } catch (error) {
    throw error.response ? error.response.data : "Something went wrong!";
